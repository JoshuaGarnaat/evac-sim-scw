...
function box(scene, size, position, color, opacity=1, group=null) {
  ...
  mesh.position.set(...position); (group || scene).add(mesh); return mesh;
}
...
function slabWithStairOpenings(scene, building, floor, elevation, group) {
  ...
  if(!stairRooms.length){box(scene,[building.dimensions.width,thickness,building.dimensions.depth],[building.dimensions.width/2,elevation-.12,building.dimensions.depth/2],color,.92,group);return;}
  ...
  let cursor=0;
  stairRooms.forEach(room=>{if(room.x>cursor)box(scene,[room.x-cursor,thickness,y1-y0],[(cursor+room.x)/2,elevation-.12,(y0+y1)/2],color,.92,group);cursor=room.x+room.width;});
...
export function renderBuilding(scene, building) {
  const groups=[new THREE.Group(),new THREE.Group(),new THREE.Group()]; groups.forEach(g=>scene.add(g));
  const h=building.dimensions.floor_height;
  for(let floor=0;floor<3;floor++){
    const elevation=floor*h; slabWithStairOpenings(scene,building,floor,elevation,groups[floor]);
  ...
  building.exits.forEach(exit=>{const side=exit.x<1||exit.x>building.dimensions.width-1,size=side?[.18,2.5,exit.width]:[exit.width,2.5,.18];const marker=box(scene,size,[exit.x,1.25,exit.y],0x37ef79,.75,groups[0]);marker.material.emissive.setHex(0x126b34);});
  building.stairs.forEach(stair=>{
    for(let floor=1;floor<=2;floor++){
      ...
      ramp(scene,stair,floor,true,h,groups[floor-1]); ramp(scene,stair,floor,false,h,groups[floor-1]);
...
export function createHeatmap(scene,building){
  ...
  for(let f=0;f<3;f++){const mesh=new THREE.InstancedMesh(geometry,new THREE.MeshBasicMaterial({vertexColors:true,transparent:true,opacity:.3,depthWrite:false}),cols*rows);let index=0;for(let y=0;y<rows;y++)for(let x=0;x<cols;x++){dummy.position.set(x*cw+cw/2,f*h+.07,y*cd+cd/2);dummy.updateMatrix();mesh.setMatrixAt(index,dummy.matrix);mesh.setColorAt(index,new THREE.Color(0x123d20));index++;}mesh.instanceMatrix.needsUpdate=true;mesh.instanceColor.needsUpdate=true;scene.add(mesh);groups.push(mesh);}
  function update(agents){const counts=new Uint16Array(3*cols*rows);agents.forEach(a=>{if(a[5]===8)return;const x=Math.max(0,Math.min(cols-1,Math.floor(a[1]/cw))),y=Math.max(0,Math.min(rows-1,Math.floor(a[2]/cd))),f=Math.max(0,Math.min(2,a[4]));counts[f*cols*rows+y*cols+x]++;});groups.forEach((mesh,f)=>{for(let i=0;i<cols*rows;i++){const v=Math.min(1,counts[f*cols*rows+i]/18);color.setHSL(.33*(1-v),.92,.18+v*.35);mesh.setColorAt(i,color);}mesh.instanceColor.needsUpdate=true;});}
  function setFloor(value){groups.forEach((mesh,f)=>mesh.visible=value==='all'||Number(value)===f);}return{groups,update,setFloor};
}
